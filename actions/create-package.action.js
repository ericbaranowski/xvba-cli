...
module.exports = async function createPackageScaffold(package) {
    try {
        const xvbaModulesDirectory = path.join(rootPath, 'xvba_modules');
        ...
        await handleCheckFolder(xvbaModulesDirectory);
        ...
        await new Promise(
            (resolve, reject) => {
                fs.writeFile(path.join(rootPath, 'config.json'), JSON.stringify(CONFIG(package), undefined, 2), (error => {
                    error ? reject(error) : resolve(true)
                }))
            }
        )
